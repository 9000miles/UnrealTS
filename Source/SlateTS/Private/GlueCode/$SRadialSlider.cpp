...
namespace $SRadialSlider
{
	static void $Arguments(const v8::FunctionCallbackInfo<v8::Value>& Info, uint8 ArgumentsIndex, v8::Local<v8::Context> Context, v8::Isolate* Isolate, SRadialSlider::FArguments& Arguments)
	{
		...
		$SLATE_ATTRIBUTE(SliderProgressColor);
		$SLATE_ATTRIBUTE(SliderHandleColor);
		$SLATE_ATTRIBUTE(CenterBackgroundColor);
		$SLATE_ATTRIBUTE(Thickness);
		$SLATE_ARGUMENT(CenterBackgroundBrush);
		$SLATE_STYLE_ARGUMENT(Style);
		$SLATE_ATTRIBUTE(StepSize);
		$SLATE_ATTRIBUTE(Value);
		$SLATE_ATTRIBUTE(bUseCustomDefaultValue);
		$SLATE_ATTRIBUTE(CustomDefaultValue);
		$SLATE_ARGUMENT(SliderRange);
		$SLATE_ARGUMENT(SliderHandleStartAngle);
		$SLATE_ARGUMENT(SliderHandleEndAngle);
		$SLATE_ARGUMENT(AngularOffset);
		$SLATE_ARGUMENT(HandStartEndRatio);
		$SLATE_ARGUMENT(ValueTags);
		$SLATE_ARGUMENT(IsFocusable);
		$SLATE_ARGUMENT(UseVerticalDrag);
		$SLATE_ARGUMENT(ShowSliderHandle);
		$SLATE_ARGUMENT(ShowSliderHand);

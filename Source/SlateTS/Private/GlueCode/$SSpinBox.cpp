...
namespace $SSpinBox
{
	static void $Arguments(const v8::FunctionCallbackInfo<v8::Value>& Info, uint8 ArgumentsIndex, v8::Local<v8::Context> Context, v8::Isolate* Isolate, SSpinBox::FArguments& Arguments)
	{
		...
		$SLATE_ATTRIBUTE(MinFractionalDigits);
		$SLATE_ATTRIBUTE(MaxFractionalDigits);
		$SLATE_ATTRIBUTE(AlwaysUsesDeltaSnap);
		$SLATE_ATTRIBUTE(EnableSlider);
		$SLATE_ATTRIBUTE(Delta);
		$SLATE_ATTRIBUTE(ShiftMouseMovePixelPerDelta);
